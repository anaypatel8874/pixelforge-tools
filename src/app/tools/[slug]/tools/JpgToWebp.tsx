  ...
  const handle = async () => {
    ...
    try {
      ...
      for (const f of files) {
        const blob = await convertImage(f, { type: 'image/webp', quality });
        out.push({ blob, name: suggestFilename(f.name, 'webp', 'webp') });
      }
      setOutput(out);
        ...
        <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-wrap gap-2">
              <button className="btn-primary" onClick={handle}><Wand2 className="h-4 w-4" /> Convert to WEBP</button>
